...
stubbed STDIO/SOCKS bindings is involved. Any caller of `lci`'s parser (including the plain CLI,
`lci <file>`) hits this on the same malformed input.
...
**How this was found**: `mayhem/build/lci -runs=200 mayhem/lci-buggy-mhh-run-17/testsuite` (letting
libFuzzer mutate the seed corpus, not just replay it) hit this within the first ~90 mutated inputs
while validating the harness's handling of the known hang at
`mayhem/lci-buggy-mhh-run-17/known-findings/gimmeh-eof-unbounded-loop/` — i.e. this is a real bug
the harness is capable of finding once that hanging seed is excluded from the sanity-probe corpus.
Minimized by hand from the fuzzer's

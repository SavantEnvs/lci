 ...
 *
 * (1) is bounded by an interpreter-internal step budget, injected into a
 ...
 * see build.sh's FUZZ_SHADOW comment for the measured per-statement cost that sets
 * this) statements/loop-iterations, interpretMainNodeScope() unwinds and
 ...
 *
 * Per PORTING.md, this harness installs no timer/alarm/watchdog of its own --
 * Mayhem owns timeouts (mayhem/Mayhemfile's `timeout: 30`). A blocked native call
 * the step budget cannot see (no interpreter statement ever dispatched) is exactly
 * the kind of hang that timeout is for, and libFuzzer already owns SIGALRM for its
 * own -timeout mechanism, so this file does not touch signals at all.
 ...
 * it is bounded the same way (1) is: by the interpreter step budget.
 *
 ...
 * budget disabled; with the step budget active, interpretStmtNode()
 ...
 * stops on an empty read) and is likewise bounded by the step budget: its
...
int LLVMFuzzerInitialize(int *argc, char ***argv)
{
	(void)argc;
	(void)argv;
	/* Defense against GIMMEH blocking: make stdin observe EOF immediately,
	 * regardless of what libFuzzer/Mayhem attaches to fd 0. Non-fatal if it
	 * fails -- the interpreter step budget still bounds a loop that calls
	 * GIMMEH repeatedly and ignores the EOF. */
	FILE *ignored = freopen("/dev/null", "r", stdin);
	(void)ignored;
	/* interpretMainNodeScope() now compiles the program to bytecode and, when a hot
	 * Proto is called with all-integer arguments, vm.c's callProto() will JIT it to
	 * real native machine code (jit.c: mmap(PROT_EXEC)/mprotect) instead of running
	 * it through vm.c's run() dispatch loop. mayhem/build.sh's VM_SHADOW step-budget
	 * check lives in run(); there is no equivalent injection point in JIT-emitted
	 * machine code, so an unbounded loop that gets JIT-compiled would bypass the
	 * budget entirely. LCI_NOJIT is an env var jit.c's jitCompileAll() already checks
	 * for exactly this purpose -- setting it keeps every call on the run() path the
	 * budget bounds, at the cost of not fuzzing the JIT codegen itself. */
	setenv("LCI_NOJIT", "1", 1);
	return 0;
}
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
	 ...
	 * interpreter step budget (see header comment) and the Mayhemfile's
	 * `timeout: 30`. A fresh, NULL scope mirrors main.c's own pipeline() call
	 * exactly, so a failure/timeout here is a genuine interpreter-level
	 * behavior, not a harness artifact of reusing state across inputs. */

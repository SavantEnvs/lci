...
static void scanExprUse(ExprNode *expr)
{
	...
	switch (expr->type) {
		...
		case ET_CAST:
			...
			break;
		case ET_SYSTEMCOMMAND:
			scanExprUse(((SystemCommandExprNode *)expr->expr)->cmd);
			break;
		case ET_FUNCCALL: {
...
static void scanStmtUse(StmtNode *node)
{
	...
	switch (node->type) {
		...
		case ST_EXPR:
			...
			break;
		case ST_IMPORT:
			/* A library defines names in the scope at run time.
			 * Nothing here declares them, so a use of one already
			 * counts as free and stays in the scope object. */
			break;
		case ST_ALTARRAYDEF: {
                       ...
                       ScopeObject *scope, ScopeObject *caller)
{
	...
	if (proto->needscope) {
		 ...
		 * points ME at the object the function was found on.  With no
		 * such object -- the main block -- the link is inherited, as
		 * interpretBlockNode does. */
		frame.owned = caller ? createScopeObjectCaller(scope, caller)
				: createScopeObject(scope);
		if (!frame.owned) {
			regstacktop = base;
			return NULL;
		}
